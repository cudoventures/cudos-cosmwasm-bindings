...
use cosmwasm_std::{
    entry_point, to_binary, wasm_execute, wasm_instantiate, AllBalanceResponse,
    AllDelegationsResponse, AllValidatorsResponse, BalanceResponse, BankMsg, BondedDenomResponse,
    Coin, CosmosMsg, Deps, DepsMut, DistributionMsg, Env, FullDelegation, GovMsg, MessageInfo,
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::SendMsg { to_address, amount } => execute_msg_send(to_address, amount),
        ExecuteMsg::BurnMsg { amount } => execute_msg_burn(amount),
        ExecuteMsg::DelegateMsg { validator, amount } => execute_msg_delegate(validator, amount),
        ExecuteMsg::UndelegateMsg { validator, amount } => {
        ...
        ExecuteMsg::SetWithdrawAddressMsg { address } => execute_msg_set_withdraw_address(address),
        ExecuteMsg::WithdrawDelegatorRewardMsg { validator } => {
            execute_msg_withdraw_delegator_reward(validator)
        }
        ExecuteMsg::VoteMsg { proposal_id, vote } => execute_msg_vote(proposal_id, vote),
...
pub fn execute_msg_send(to_address: String, amount: Vec<Coin>) -> StdResult<Response> {
    let send = BankMsg::Send { to_address, amount };
    let msg: CosmosMsg = send.clone().into();
    Ok(Response::new().add_message(msg))
}

pub fn execute_msg_burn(amount: Vec<Coin>) -> StdResult<Response> {
    let burn = BankMsg::Burn { amount };
    let msg: CosmosMsg = burn.clone().into();
    Ok(Response::new().add_message(msg))
}

pub fn execute_msg_delegate(validator: String, amount: Coin) -> StdResult<Response> {
    let delegate = StakingMsg::Delegate { validator, amount };
    let msg: CosmosMsg = delegate.clone().into();
    Ok(Response::new().add_message(msg))
}

pub fn execute_msg_undelegate(validator: String, amount: Coin) -> StdResult<Response> {
...
pub fn execute_msg_redelegate(
    ...
    let redelegate = StakingMsg::Redelegate {
        src_validator,
        dst_validator,
        amount,
    };
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<QueryResponse> {
    match msg {
        ...
        QueryMsg::AllDelegations { delegator } => {
            to_binary(&query_all_delegations(deps, delegator)?)
        }
        QueryMsg::Delegation {
            delegator,
            validator,
        } => to_binary(&query_delegation(deps, delegator, validator)?),
...
pub fn query_all_balances(deps: Deps, address: String) -> StdResult<AllBalanceResponse> {
    let amount = deps.querier.query_all_balances(address)?;
    Ok(AllBalanceResponse { amount })
}

pub fn query_bonded_denom(deps: Deps) -> StdResult<BondedDenomResponse> {
    let denom = deps.querier.query_bonded_denom()?;
    Ok(BondedDenomResponse { denom })
}

pub fn query_all_delegations(deps: Deps, delegator: String) -> StdResult<AllDelegationsResponse> {
...
pub fn query_all_validators(deps: Deps) -> StdResult<AllValidatorsResponse> {
